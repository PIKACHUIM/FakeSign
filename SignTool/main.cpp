...
LPCWSTR ReplaceTimeStamp(LPCWSTR lpOriginalTS) {
	...
	if (!_wcsicmp(lpOriginalTS, L"{CustomTimestampMarker-SHA1}")) {
		//wcscat(buf, L"http://localhost:1003/TSA/");
		wcscat(buf, lpServerURL);
...
bool ParseConfig(LPWSTR lpCommandLineConfig, LPWSTR lpCommandLineTimestamp)
{
	...
	if (lpCommandLineTimestamp)
		...
		GetPrivateProfileStringW(L"TimeStamp", L"ServerURL", NULL, lpServerURL, 256, buf);
	}
	swscanf(lpTimestamp, L"%d-%d-%dT%d:%d:%d", &year, &month, &day, &hour, &minute, &second);
	//year = _wtoi(lpTimestamp);
...
)
{
	if (fdwReason == DLL_PROCESS_ATTACH)
	{
		...
		WCHAR output[300];
		WCHAR setfor[20];
		_swprintf(setfor, L"%4d-%02d-%02dT%02d:%02d:%02d", year, month, day, hour, minute, second);
		wcscat(output, (WCHAR*)L"自定义时间戳为: ");
		wcscat(output, (WCHAR*)lpTimestamp);
		wcscat(output, (WCHAR*)L"\n自定义显示时间: ");
		wcscat(output, (WCHAR*)setfor);
		wcscat(output, (WCHAR*)L"\n自定义服务器为: ");
